...
class SauceNao:
	...
	async def from_url(self, url: str) -> SauceNaoResults:
		...
		status_code, response = await self._fetch(self.API_URL, params)

		self._verify_request(status_code, response)
		return SauceNaoResults(response, self._session, self._min_similarity, self._priority, self._priority_tolerance)
	...
	async def from_file(self, path_or_fh: Union[str, typing.BinaryIO]) -> SauceNaoResults:
		...
		async def _post(_fh: typing.IO):
			...
			_status_code, _response = await self._post(self.API_URL, params)
	...
	async def _fetch(self, url: str, params: Optional[Mapping[str, str]] = None) -> Tuple[int, dict]:
		async with self._session.get(url, params=params) as response:
			return response.status, await response.json()

	async def _post(self, url: str, params: Optional[Mapping[str, str]] = None) -> Tuple[int, dict]:
