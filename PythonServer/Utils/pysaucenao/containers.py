...
from .errors import SauceNaoException
...
class SauceNaoResults:
	...
	def __init__(self, response: dict, session: aiohttp.ClientSession, min_similarity: typing.Optional[float] = None,
	...
	def _process_result(self, result):
		...
		if header['index_id'] in [21, 22]:
			return AnimeSource(header, data, self._session)
	...
	def _sort_results(self) -> None:
		...
		if self._min_similarity:
			self._results = [r for r in self._results if float(
				r['header']['similarity']) > self._min_similarity]
		...
		if self._priority:
			...
			extra_results.sort(key=lambda x: float(
				x['header']['similarity']), reverse=True)

			# Now make sure the priority indexes are sorted
			for _index_id in priority_index.keys():
				priority_index[_index_id].sort(key=lambda x: float(
					x['header']['similarity']), reverse=True)
	...
	def __repr__(self):
		return f"<SauceNaoResults(count={len(self.results)}, short_avail={self.short_remaining}, long_avail={self.long_remaining}, results={self.results})>"
...
class GenericSource:
	...
	def __init__(self, header: dict, data: dict):
		...
		self.hidden:        typing.Optional[bool] = None
		...
		# The name of the index pulled from. See INDEXES above
	...
	def _parse_header(self, header: dict):
		...
		self.hidden = bool(header['hidden'])
	...
	def _parse_data(self, data: dict):
		...
		if 'member_name' in data:
			...
			self.author_name = data['creator'][0] if isinstance(
				data['creator'], list) else data['creator']
	...
	def __repr__(self):
		rep = reprlib.Repr()
		rep.maxstring = 10
		return f"<GenericSource(t={rep.repr(self.title)}, a={rep.repr(self.author_name)}, s='{self.index}', h={self.hidden}, sim={self.similarity})>"
...
class PixivSource(GenericSource):
	...
	def _parse_data(self, data: dict):
		...
		self.pixiv_id = data.get('pixiv_id')
		self.member_id = data.get('member_id')

	def __repr__(self):
		rep = reprlib.Repr()
		rep.maxstring = 10
		return f"<PixivSource(t={rep.repr(self.title)}({self.pixiv_id}), a={rep.repr(self.author_name)}({self.member_id}), s='{self.index}', h={self.hidden}, sim={self.similarity})>"
...
class BooruSource(GenericSource):
	...
	def __repr__(self):
		rep = reprlib.Repr()
		rep.maxstring = 10
		return f"<BooruSource(t={rep.repr(self.title)}({self.gelbooru_id},{self.danbooru_id}), a={rep.repr(self.author_name)}, s='{self.index}', h={self.hidden}, sim={self.similarity})>"
...
class TwitterSource(GenericSource):
	...
	def __repr__(self):
		rep = reprlib.Repr()
		rep.maxstring = 10
		return f"<TwitterSource(t={rep.repr(self.title)}({self.tweet_id}), a={rep.repr(self.author_name)}({self.twitter_user_id}), s='{self.index}', h={self.hidden}, sim={self.similarity})>"
...
class VideoSource(GenericSource):
	...
	def __repr__(self):
		rep = reprlib.Repr()
		rep.maxstring = 10
		return f"<GenericSource(t={rep.repr(self.title)}, e={self.episode}, s='{self.index}', h={self.hidden}, sim={self.similarity})>"
...
class AnimeSource(VideoSource):
	...
	def __init__(self, header: dict, data: dict, session: aiohttp.ClientSession):
		self._ids = None
		self._session = session
	...
	async def load_ids(self) -> typing.Dict[str, int]:
		...
		self._ids = {}
		try:
			response = await self._session.get(f"https://relations.yuna.moe/api/ids?source=anidb&id={self.data.get('anidb_aid')}")
	...
	def __repr__(self):
		rep = reprlib.Repr()
		rep.maxstring = 10
		return f"<AnimeSource(t={rep.repr(self.title)}, e={self.episode}, loaded='{self._ids is not None}', s='{self.index}', h={self.hidden}, sim={self.similarity})>"
...
class MangaSource(GenericSource):
	...
	def __repr__(self):
		rep = reprlib.Repr()
		rep.maxstring = 10
		return f"<MangaSource(t={rep.repr(self.title)}, a={rep.repr(self.author_name)}, c={self.chapter}, s='{self.index}', h={self.hidden}, sim={self.similarity})>"
