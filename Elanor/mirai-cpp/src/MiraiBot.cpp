...
namespace Cyan
{
    ...
    void MiraiBot::Connect(const SessionOptions& opts)
    {
        ...
        pmem->httpClient->set_read_timeout(opts.HttpReadTimeout.Get());
        pmem->httpClient->set_write_timeout(opts.HttpWriteTimeout.Get());
    ...
    GroupAnnouncement MiraiBot::PublishGroupAnnouncement(const GID_t& group, 
                                                         const string& content, 
                                                         int flags, 
                                                         const std::optional<MiraiImage>& image)
    {
        json data =
        {
            { "sessionKey", pmem->sessionKey },
            { "target", int64_t(group) },
            { "content", content },
            { "pinned", (bool)(flags & GroupAnnouncement::PublishFlags::Pinned) },
            { "showEditCard", (bool)(flags & GroupAnnouncement::PublishFlags::ShowEditCard) },
            { "showPopup", (bool)(flags & GroupAnnouncement::PublishFlags::ShowPopup) },
            { "requireConfirmation", (bool)(flags & GroupAnnouncement::PublishFlags::RequireConfirmation) },
            { "sendToNewMember", (bool)(flags & GroupAnnouncement::PublishFlags::SendToNewMember) }
        };
        if (image.has_value())
        {
            image->Url.empty()
                ? data["imageUrl"] = json(nullptr)
                : data["imageUrl"] = image->Url;
            image->Path.empty()
                ? data["imagePath"] = json(nullptr)
                : data["imagePath"] = image->Path;
            image->Base64.empty()
                ? data["imageBase64"] = json(nullptr)
                : data["imageBase64"] = image->Base64;
        }
        auto res = pmem->httpClient->Post("/anno/publish", data.dump(), CONTENT_TYPE.c_str());
        auto re_json = ParseOrThrowException(res);
        GroupAnnouncement result;
