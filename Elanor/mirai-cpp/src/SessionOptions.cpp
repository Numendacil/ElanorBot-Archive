...
namespace Cyan
{
	...
	SessionOptions SessionOptions::FromJson(const json& json)
	{
		...
		if (json.contains("http-read-timeout"))
		{
			opt.HttpReadTimeout = json["http-read-timeout"].get<long>();
		}
		if (json.contains("httpReadTimeout"))
		{
			opt.HttpReadTimeout = json["httpReadTimeout"].get<long>();
		}
		if (json.contains("http-write-timeout"))
		{
			opt.HttpReadTimeout = json["http-write-timeout"].get<long>();
		}
		if (json.contains("httpWriteTimeout"))
		{
			opt.HttpReadTimeout = json["httpWriteTimeout"].get<long>();
		}
		return opt;
	}
}