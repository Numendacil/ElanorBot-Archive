...
bool SetTrigger::Parse(const Cyan::MessageChain& msg, vector<string>& tokens)
{
...
bool SetTrigger::Execute(const Cyan::GroupMessage& gm, Bot::Group& group, const vector<string>& tokens) 
{
	...
	if (command == "set" || command == "status" || command == "list")
	{
		...
		if (tokens.size() < 3)
		{
			...
			client.Send(gm.Sender.Group.GID, Cyan::MessageChain().Plain("缺少参数[trigger]，是被你吃了嘛"));
			return false;
		}
		...
		if (!trigger_status->ExistTrigger(target))
		{
			...
			client.Send(gm.Sender.Group.GID, Cyan::MessageChain().Plain(target + "是哪个触发器捏，不知道捏"));
			return false;
		}
		...
		if (command == "status")
		{
			...
			client.Send(gm.Sender.Group.GID, Cyan::MessageChain().Plain(target + " 当前状态: " + string((trigger_status->GetTriggerStatus(target))? "✅" : "❌")));
			return true;
		}
		...
		if (tokens.size() < 4)
		{
			...
			client.Send(gm.Sender.Group.GID, Cyan::MessageChain().Plain("on还是off, 到底要选哪个呢🔉到底要选哪个呢🔉到底要选哪个呢🔉"));
			return false;
		}
		...
		case 1:
			...
			client.Send(gm.Sender.Group.GID, Cyan::MessageChain().Plain("已启动 " + target + " ✅"));
			return true;

		case 0:
			trigger_status->UpdateTriggerStatus(target, false);
			logging::INFO("Trigger off ❌ <SetTrigger>: " + target + Utils::GetDescription(gm, false));
			client.Send(gm.Sender.Group.GID, Cyan::MessageChain().Plain("已关闭 " + target + " ❌"));
			return true;

		default:
			logging::INFO("未知选项 <SetTrigger>: " + tokens[3] + Utils::GetDescription(gm, false));
			client.Send(gm.Sender.Group.GID, Cyan::MessageChain().Plain(tokens[3] + "是什么意思捏，看不懂捏"));
			return false;
		}
	}
	...
	client.Send(gm.Sender.Group.GID, Cyan::MessageChain().Plain(tokens[1] + "是什么东西捏，不知道捏"));
	return false;
}
