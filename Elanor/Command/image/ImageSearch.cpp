...
bool ImageSearch::Execute(const Cyan::GroupMessage& gm, Group& group, const vector<string>& tokens) 
{
	...
	if (tokens.size() > 1)
	{
		...
		else
		{
			...
			Utils::SendGroupMessage(gm, Cyan::MessageChain().Plain(tokens[1] + "是什么搜索引擎捏，不知道捏"));
			return false;
		}
	}
	...
	else
	{
		...
		if (quote.empty())
		{
			...
			Utils::SendGroupMessage(gm, Cyan::MessageChain().Plain("图捏"));
			return false;
		}
		try
		{
		...
		catch (MiraiApiHttpException &)
		{
			...
			Utils::SendGroupMessage(gm, Cyan::MessageChain().Plain("消息太久了，看不到是什么图捏"));
			return false;
		}
	}
