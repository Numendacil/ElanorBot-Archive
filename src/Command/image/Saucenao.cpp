...
{
	logging::INFO("Calling Saucenao <Saucenao>" + Utils::GetDescription(gm));
	if (token.size() > 1)
	{
		if (token[1] == "help" || token[1] == "h" || token[1] == "帮助")
		{
			logging::INFO("帮助文档 <Saucenao>" + Utils::GetDescription(gm, false));
	...
	else
	{
		...
		if (quote.empty())
		{
			logging::INFO("格式错误 <Saucenao>: 未附带图片或回复" + Utils::GetDescription(gm, false));
		...
		try
		{
			...
			{
				logging::INFO("格式错误 <Saucenao>: 回复内容不包含图片" + Utils::GetDescription(gm, false));
				Utils::SendGroupMessage(gm, MessageChain().Plain("图捏"));
				return false;
			}
		}
		...
		{
			logging::INFO("无法从回复内容获得图片 <Saucenao>" + Utils::GetDescription(gm, false));
	...
	if (remaining > 0s)
	{
		stringstream ss;
		ss << remaining;
		logging::INFO("冷却剩余 <Saucenao>: " + ss.str() + Utils::GetDescription(gm, false));
	...
	logging::INFO("上传结果 <Saucenao>" + Utils::GetDescription(gm, false));
